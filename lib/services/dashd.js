...
Dash.DEFAULT_CONFIG_SETTINGS = {
  ...
  zmqpubrawtxlock: 'tcp://127.0.0.1:38332',
...
Dash.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = path.resolve(spawnOptions.datadir, './but.conf');
...
Dash.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === dashcore.Networks.testnet) {
    networkPath = 'testnet3/but.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/but.conf';
    }
  }
  return networkPath;
};
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('ButK Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Dash.prototype.getMNList = function(callback){
  ...
  var getRank = function(next){
    self.client.smartnodelist('rank', function(err, response){
      if(response && response.hasOwnProperty('result')){
        var result = response.result;
  ...
  var getProtocol = function(next){
	  self.client.smartnodelist('protocol', function(err, response){
		  if(response && response.hasOwnProperty('result')){
			  var result = response.result;
  ...
  var getPayee = function(next){
	  self.client.smartnodelist('payee', function(err, response){
		  if(response && response.hasOwnProperty('result')){
			  var result = response.result;
  ...
  var getLastSeen = function(next){
	  self.client.smartnodelist('lastseen', function(err, response){
		  if(response && response.hasOwnProperty('result')){
			  var result = response.result;
  ...
  var getActiveSeconds=function(next){
	  self.client.smartnodelist('activeseconds', function(err, response){
		  if(response && response.hasOwnProperty('result')){
			  var result = response.result;
  ...
  var getIP = function(next){
	  self.client.smartnodelist('addr', function(err, response){
		  if(response && response.hasOwnProperty('result')){
			  var result = response.result;
  ...
  var getStatus = function(next){
	  self.client.smartnodelist('status', function(err, response){
		  if(response && response.hasOwnProperty('result')){
			  var result = response.result;
  ...
  var prepareResponse = function(err){
    ...
    if(
        ...
        for(var i = 0; i<rankLength; i++){
          ...
          return callback(new Error('Invalid smartnodeList'),null);
    }
	  self.smartnodeListCache.set('', MNList);
	  return callback(null, MNList);
  };
  var MNListData = self.smartnodeListCache.get('');
  if (MNListData) {
      return setImmediate(function() {
...
Dash.prototype.getInfo = function(callback) {
  ...
  client.getNetworkInfo(function (err, networkResponse){
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    const networkInfo = networkResponse.result;
    client.getBlockchainInfo(function (err, blockchainResponse){
      if(err){
        return callback(self._wrapRPCError(err));
      }
      const blockchainInfo = blockchainResponse.result;
      const info = {
        bestBlockHash: blockchainInfo.bestblockhash,
        blocks: blockchainInfo.blocks,
        chain: blockchainInfo.chain,
        difficulty: blockchainInfo.difficulty,
        errors: blockchainInfo.warnings,
